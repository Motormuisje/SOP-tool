...
| 12 | **Wat-als-werkstroom op de bemensingsnormen** (klantvraag 2026-08-06: "aanpassen, Enter, meteen resultaat, verder spelen — en dan opslaan als scenario of in de config"). Een normwijziging wordt sessie-wat-als: rekent DIRECT mee (POST `/api/fte/norm_overrides`), raakt de masterdata niet, en draagt bron **"wat-als"** zodat een experiment nooit voor een vastgelegde norm kan doorgaan. Het paneel rechtsonder houdt de wijzigingen bij (oud → nieuw) met de twee uitgangen: **Opslaan in masterdata** (bestaande PATCH met versiecontrole; daarna gaat de wat-als vanzelf uit) of **Terugzetten**; een scenario opslaan bewaart de stand automatisch (de overrides reizen door alle zes sync-/rebuildpunten, exact als de actieve combinaties). De vergelijking rekent alle varianten mét de sessie-wat-als, zodat alleen de combinatieset verschilt. Verhouding tot de eerdere afwijzing "norm-wat-als per variant": die blijft staan — dit is één sessiebrede stand met bronlabel en persistentie, geen normen die per vergelijkingsvariant verschillen en nergens bestaan | sessiestate (zes syncpunten) |
| 13 | **Keuzelijsten op alle sleutel-invoer** (klantvraag 2026-08-06: "teveel risico op een verkeerde naam of sleutel intypen"). "+ rij" opent een sleutelkiezer i.p.v. een prompt; paarsleutels (MACHINE\|MATERIAAL) zijn twee velden met elk hun eigen lijst; zoeken werkt op een deel van naam óf code; een suggestieveld weigert codes die niet in de lijst staan (vrije ID's zoals een nieuwe combinatie blijven vrij). Ook: wizard-materiaalnummer/familie, csv-machinecellen (per token, reeds gekozen codes niet opnieuw aangeboden), configvelden unlimited-machines en purchased&produced, en de MES-overname (materiaal wordt gekozen, niet getypt) | invulveiligheid op bestaande paden |
