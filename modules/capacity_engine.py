...
class CapacityEngine:
    def __init__(self, data: DataLoader, production_plan: Dict[str, Dict[str, float]],
                 ...
                 overrides: Dict[str, Dict[str, Dict[str, float]]] = None):
        self.data = data
        self.periods = data.periods
        self.production_plan = production_plan
        # User-supplied overrides for L7/L9/L11/L12.
        # Structure: overrides[line_type_value][group_or_machine_code][period] = float
        # Applied AFTER all engine calculations (incl. site exceptions) in
        # _apply_user_overrides; L10 is then recomputed from overridden L7/L9.
        self.overrides: Dict[str, Dict[str, Dict[str, float]]] = overrides or {}
    ...
    def calculate(self) -> Dict[str, List[PlanningRow]]:
        ...
        # User overrides win over both base calc and site exceptions. Apply the
        # capacity-driver overrides first, then recompute derived rows from the
        # override-aware structures. L12 overrides are leaf edits and are
        # applied after L12 is regenerated.
        ...
        self._refresh_l9_aux2()
        self._recalculate_utilization_rate()
        self._recalculate_fte_requirements()
        self._apply_user_overrides({LineType.FTE_REQUIREMENTS.value})
    ...
    def _apply_override(self, line_type: str, code: str, period: str,
                        computed_value: float) -> float:
        """Return the override value if one exists for (line_type, code, period),
        else the computed_value. Never raises on missing keys."""
        return (
            self.overrides
                .get(line_type, {})
                .get(code, {})
                .get(period, computed_value)
        )

    def _apply_user_overrides(self, line_types=None):
        """Apply user-supplied overrides to L7/L9/L11/L12 in-place.

        Overrides win over base calc and site exceptions. Missing entries are
        silent no-ops; rows whose code isn't in overrides are untouched.
        """
        line_to_rows = {
            LineType.CAPACITY_UTILIZATION.value: self.rows_07_cap,
            LineType.AVAILABLE_CAPACITY.value: self.rows_09,
        ...
        for line_type, rows in line_to_rows.items():
            if line_types is not None and line_type not in line_types:
                continue
            line_overrides = self.overrides.get(line_type)
            if not line_overrides:
                continue
            for row in rows:
                # For L7 we override only group-level rows (product_type ==
                # 'Machine Group') so user edits act on the aggregate hours
                # surfaced in the UI, not on individual material/machine rows.
                if line_type == LineType.CAPACITY_UTILIZATION.value \
                        and row.product_type != 'Machine Group':
                    continue
                code = row.material_number
                code_overrides = line_overrides.get(code)
                if not code_overrides:
                    continue
                for period, override_val in code_overrides.items():
                    if period in row.values:
                        row.values[period] = override_val
                        if line_type == LineType.CAPACITY_UTILIZATION.value:
                            self.group_hours_aggregated.setdefault(code, {})[period] = override_val

        if line_types is None or LineType.CAPACITY_UTILIZATION.value in line_types:
            self._redistribute_l7_to_machine_rows()
        if line_types is None or LineType.SHIFT_AVAILABILITY.value in line_types:
            self._refresh_l9_aux_from_l11()

    def _redistribute_l7_to_machine_rows(self):
        """Mirror group-level L7 overrides into per-machine L7 rows pro-rata.

        Keeps the displayed per-machine hours consistent with the edited group
        total, so the UI/Excel don't show stale machine values that no longer
        sum to the group row. Idempotent: after the first pass each machine's
        share already matches the override, so subsequent passes are no-ops.
        """
        group_overrides = self.overrides.get(LineType.CAPACITY_UTILIZATION.value) or {}
        if not group_overrides:
            return

        machines_by_group: Dict[str, List[PlanningRow]] = defaultdict(list)
        for row in self.rows_07_cap:
            if row.product_type != 'Machine':
                continue
            machine_code = row.material_name
            machine = self.data.machines.get(machine_code)
            group = (machine.machine_group if machine else '') \
                or row.product_family or row.aux_column or ''
            if group:
                machines_by_group[group].append(row)

        for group_id, period_overrides in group_overrides.items():
            machine_rows = machines_by_group.get(group_id, [])
            if not machine_rows:
                continue
            for period, override_val in period_overrides.items():
                baseline_total = sum(r.values.get(period, 0.0) for r in machine_rows)
                if baseline_total > 0:
                    for r in machine_rows:
                        old_val = r.values.get(period, 0.0)
                        r.values[period] = old_val / baseline_total * override_val
                else:
                    split_val = override_val / len(machine_rows)
                    for r in machine_rows:
                        r.values[period] = split_val

    def _refresh_l9_aux_from_l11(self):
        """Refresh each L9 machine row's aux_column to show the (possibly
        overridden) shift hours from its group's L11 row. Aux is a single
        scalar, so we show the average across periods — matches the way L9
        already averages availability into aux_2_column.
        """
        l11_avg_by_group: Dict[str, float] = {}
        for row in self.rows_11:
            vals = list(row.values.values())
            if vals:
                l11_avg_by_group[row.material_number] = sum(vals) / len(vals)

        for row in self.rows_09:
            machine = self.data.machines.get(row.material_name)
            if not machine or machine.shift_system == ShiftSystem.UNLIMITED:
                continue
            shift_h = l11_avg_by_group.get(machine.machine_group)
            if shift_h is None:
                continue
            row.aux_column = (str(int(shift_h)) if shift_h == int(shift_h)
                              else str(round(shift_h, 1)))

    def _refresh_l9_aux2(self):
        """Recompute aux_2_column (average availability) for each L9 row from
        current values. Must be called after _apply_user_overrides so period
        overrides are reflected in the displayed average."""
        for row in self.rows_09:
            vals = [v for v in row.values.values() if v is not None]
            if vals:
                avg = sum(vals) / len(vals)
                row.aux_2_column = str(avg)

    def _machine_hours_for_utilization(self) -> Dict[str, Dict[str, float]]:
        """Return machine cap-util hours used by L10 recalc.

        Reads directly from the L7 machine rows, which already reflect any
        L7 group overrides redistributed by ``_redistribute_l7_to_machine_rows``.
        """
        return {
            row.material_name: dict(row.values)
            for row in self.rows_07_cap
            if row.product_type == 'Machine'
        }

    def _recalculate_utilization_rate(self):
        """Re-derive L10 from (possibly overridden) L7 group hours and L9
        machine availability. Replaces the values produced by the first pass
        so user overrides on L7/L9 propagate to L10 without altering the
        machine-level structure of L10 rows.
        """
        # Build lookup: machine_code -> override-aware cap-util hours per period.
        machine_hours = self._machine_hours_for_utilization()

        # Build lookup: machine_id -> availability per period from L9
        avail_by_machine_id: Dict[str, Dict[str, float]] = {}
        for row in self.rows_09:
            avail_by_machine_id[row.material_number] = dict(row.values)

        for row in self.rows_10:
            machine_code = row.material_name
            machine = self.data.machines.get(machine_code)
            if not machine:
                continue
            if machine.shift_system == ShiftSystem.UNLIMITED:
                continue  # already 1.0; not driven by L7/L9
            avail = avail_by_machine_id.get(machine.machine_id, {})
            used = machine_hours.get(machine_code, {})
            new_rate = {}
            for period in self.periods:
                shift_hours = self._get_shift_hours_for_machine(machine_code, period)
                avail_factor = avail.get(period, machine.get_availability(period))
                available = shift_hours * avail_factor
                used_h = used.get(period, 0.0)
                new_rate[period] = used_h / available if available > 0 else 0.0
            row.values = new_rate
            vals = list(new_rate.values())
            row.aux_column = str(sum(vals) / len(vals) if vals else 0.0)

    def _get_shift_hours_for_machine(self, machine_code: str, period: str = None) -> float:
        """Return monthly shift hours for a machine, driven by FTE sheet values.

        Precedence: L11 group/period override (planning tab edit) > per-machine
        shift_hours_override (machines tab edit) > shift-system default lookup.
        """
        _KEY = {
        ...
        if period is not None:
            group_overrides = self.overrides.get(LineType.SHIFT_AVAILABILITY.value) or {}
            group_id = machine.machine_group
            if group_id in group_overrides and period in group_overrides[group_id]:
                return float(group_overrides[group_id][period])
    ...
    def _calculate_shift_availability(self):
        ...
        for group_id in self.all_groups:
            ...
            machine_names = []
            group_shift_hours = grp_shift_hours
            has_machine_shift_override = False
            if group:
                ...
                override_hours = [
                    self._get_shift_hours_for_machine(mc)
                    for mc in group.machine_codes
                    if mc in self.data.machines
                    and getattr(self.data.machines[mc], 'shift_hours_override', None) is not None
                ]
                has_machine_shift_override = bool(override_hours)
                if has_machine_shift_override:
                    machine_hours = [
                        self._get_shift_hours_for_machine(mc)
                        for mc in group.machine_codes
                        if mc in self.data.machines
                    ...
                    if machine_hours:
                        group_shift_hours = sum(machine_hours) / len(machine_hours)
            annual_hours = group_shift_hours * 12
            self.group_monthly_shift_hours[group_id] = group_shift_hours
            ...
            self.rows_11.append(PlanningRow(
                ...
                aux_column='Machine override' if has_machine_shift_override else grp_shift_name,
                aux_2_column=aux2_str,
                values={p: group_shift_hours for p in self.periods}
            ))
        ...
        for machine_code, machine in self.data.machines.items():
            ...
            for period in self.periods:
                if machine.shift_system == ShiftSystem.UNLIMITED:
                    ...
                    machine_shift_hours = self._get_shift_hours_for_machine(machine_code, period)
    ...
    def _recalculate_fte_requirements(self):
        self.rows_12 = []
