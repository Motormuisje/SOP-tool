...
class DataLoader:
    ...
    def _load_machines(self):
        ...
        for _, row in df.iterrows():
            ...
            if self.periods:
                first_avail = next(iter(avail.values()), 1.0)
                last_avail = first_avail
                for ps in self.periods:
                    if ps in avail:
                        last_avail = avail[ps]
                    else:
                        avail[ps] = last_avail
    ...
    def _extend_machine_availability_to_periods(self):
        for machine in self.machines.values():
            avail = dict(getattr(machine, 'availability_by_period', {}) or {})
            if not self.periods:
                machine.availability_by_period = avail
                continue
            known = sorted(avail.items())
            if not known:
                machine.availability_by_period = {ps: 1.0 for ps in self.periods}
                continue
            known_idx = 0
            last_avail = known[0][1]
            for ps in self.periods:
                while known_idx < len(known) and known[known_idx][0] <= ps:
                    last_avail = known[known_idx][1]
                    known_idx += 1
                if ps not in avail:
                    avail[ps] = last_avail
            machine.availability_by_period = avail
