...
class PlanningEngine:
    ...
    def run(self) -> 'PlanningEngine':
        ...
        if self.planning_month:
            # VBA DefineVariables lines 3447-3449 anchor forecast columns
            ...
            # VBA DefineVariables lines 3447-3449 use ForecastActualsMonths
        ...
        # VBA DefineVariables line 3443 sets end column from ForecastMonths only:
        ...
        if hasattr(self.data, '_extend_machine_availability_to_periods'):
            self.data._extend_machine_availability_to_periods()
    ...
    def to_excel_with_values(self, output_path: str, inventory_quality_engine=None, previous_cycle_df=None):
        ...
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            ...
            _overview_chart_w_cm = 13.5
            _overview_chart_h_cm = 8.0
            _overview_wide_chart_w_cm = 15.5
            _overview_wide_chart_h_cm = 8.8
            _ov_chart2_anchor = 'A' + str(_chart_base + 22)   # Inventory Quality (Phase 2)
            ...
            try:
                ...
                if _iq_instance is not None:
                    ...
                    if top10:
                        ...
                        top10_period_keys = ['Starting stock'] + list(t10_periods)
                        ...
                        for ri, p in enumerate(top10_period_keys, start=2):
                            ws_t10.cell(row=ri, column=1, value=p)
                            for mi, item in enumerate(top10_sorted, start=2):
                                overstock_by_period = item.get('overstock_by_period', {})
                                if p in overstock_by_period:
                                    val = overstock_by_period.get(p, 0)
                                else:
                        ...
                        for _item in top10_sorted:
                            _vals = []
                            for _p in top10_period_keys:
                                _overstock_by_period = _item.get('overstock_by_period', {})
                                if _p in _overstock_by_period:
                                    _vals.append(float(_overstock_by_period.get(_p, 0) or 0))
                                else:
                        ...
                        _t10_img = _XLImage(_cr.top10_overstocks(top10_period_keys, _t10_mats))
                        max_row = 1 + len(top10_period_keys)
            ...
            try:
                ...
                if _iq2 is not None:
                    ...
                    # `period_totals` intentionally excludes the 'Starting stock' column
                    # (engine contract — see test_period_totals_exclude_starting_stock),
                    # but the chart needs it as the leading column to match the web UI.
                    # Aggregate each material's 'Starting stock' category here.
                    _per_material = iq_data2.get('per_material', [])
                    _ss_totals = {'under': 0.0, 'safety': 0.0, 'strategic': 0.0,
                                  'normal': 0.0, 'overstock': 0.0, 'inventory': 0.0}
                    for _m in _per_material:
                        _ss = (_m.get('periods', {}) or {}).get('Starting stock')
                        if _ss:
                            for _cat in _ss_totals:
                                _ss_totals[_cat] += _ss.get(_cat, 0.0)

                    def _iq_totals(p):
                        return _ss_totals if p == 'Starting stock' else period_totals.get(p, {})

                    if period_totals and iq_periods:
                        # Prepend the starting-stock column so the chart opens on the
                        # initial inventory position, exactly like the web UI chart.
                        iq_periods = ['Starting stock'] + list(iq_periods)
                        ...
                        for ri, p in enumerate(iq_periods, start=2):
                            pt = _iq_totals(p)
                            # COGS has no 'Starting stock' value; leave that cell blank.
                            cogs_val = None
                            if p != 'Starting stock' and cogs_row and p in cogs_row.values:
                                try:
                            ...
                            row_vals = [
                                ...
                                round(cogs_val, 0) if cogs_val is not None else None,
                            ]
                        ...
                        _iq_bands = {
                            band: [round(_iq_totals(p).get(band, 0), 0)
                                   for p in iq_periods]
                            for band in ('under', 'safety', 'strategic', 'normal', 'overstock')
                        }
                        _iq_actual = [round(_iq_totals(p).get('inventory', 0), 0)
                                      for p in iq_periods]
                        _iq_cogs = []
                        for _p in iq_periods:
                            if _p == 'Starting stock':
                                # No COGS for the starting column → gap in the line.
                                _iq_cogs.append(float('nan'))
                                continue
                            try:
            ...
            try:
                if iq_chart_done:
                    _ov_iq_img = _XLImage(_cr.inventory_quality(
                        iq_periods, _iq_bands, _iq_actual, _iq_cogs,
                        width_cm=_overview_wide_chart_w_cm, height_cm=_overview_wide_chart_h_cm,
                    ))
            ...
            try:
                if top10_count:
                    _ov_t10_img = _XLImage(_cr.top10_overstocks(
                        top10_period_keys, _t10_mats,
                        width_cm=_overview_wide_chart_w_cm, height_cm=_overview_wide_chart_h_cm,
                    ))
            ...
            try:
                ...
                _mom_result = MoMComparisonEngine.calculate_sequential(df_volumes, 6)
                if _mom_result.get('available'):
                    _mom_rows = []
                    for _transition in _mom_result.get('transitions', []):
                        for _row in _transition.get('rows', []):
                            _mom_rows.append({
                                'Material number': _row.get('material_number'),
                                'Material name': _row.get('material_name'),
                                'From Period': _transition.get('from_period'),
                                'To Period': _transition.get('to_period'),
                                'From Inventory': _row.get('from_inventory'),
                                'To Inventory': _row.get('to_inventory'),
                                'Delta': _row.get('delta'),
                                'Delta %': _row.get('delta_pct'),
                            })
                    ...
                    if not _mom_df.empty:
                        ...
                        # --- scatter chart: Starting stock vs target inventory ---
                        _scatter_data = _mom_result.get('scatter', {})
                        if _scatter_data.get('materials'):
