...
Quick fixture-free CI slice:

```powershell
pytest -m no_fixture -v
```
...
CI-compatible coverage, including XML:
...
to `htmlcov/`, and `coverage.xml` is written for CI/external tools. Both are
local outputs and are not committed.

## Browser Validation

Browser tests require Playwright and the golden fixture because they start a
real Flask server subprocess and upload/calculate the workbook before testing
the UI.
...
These tests validate product behavior but do not normally count toward Python
coverage, because the web app code runs in a subprocess.

## Performance Guardrails

Performance tests are marked separately so they can be run on demand or in a
nightly job without slowing the fixture-free CI slice:
