...
class TestShiftAvailability:
    ...
    def test_machine_shift_hours_override_updates_group_shift_availability(self):
        data, plan = _minimal_setup()
        data.machines["MC1"].shift_hours_override = 640.0
        ...
        avail_rows = results[LineType.SHIFT_AVAILABILITY.value]
        grp_row = next(r for r in avail_rows if r.material_number == "ZZ_GROUP01")
        assert grp_row.values["2025-01"] == pytest.approx(640.0)
        assert grp_row.aux_2_column == "7680"
