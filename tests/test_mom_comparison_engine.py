...
def _inv_df(materials, periods=None, line_type=INV_LINE):
    ...
    for mat_num, values in materials.items():
        row = {
            ...
            "Line type": line_type,
            "Starting stock": values[0] if values else 0.0,
        }
...
class TestCalculateSequential:
    ...
    def test_returns_available_false_with_no_periods(self):
        df = pd.DataFrame([{
            "Material number": "MAT-1",
            "Material name": "Mat MAT-1",
            "Product type": "Bulk Product",
            "Line type": INV_LINE,
            "Starting stock": 100.0,
        }])
        result = MoMComparisonEngine.calculate_sequential(df)
        assert result["available"] is False
    ...
    def test_summary_delta_computed_correctly(self):
        ...
        assert s["end"] == pytest.approx(150.0)
        assert s["delta"] == pytest.approx(50.0)

    def test_num_months_compares_starting_stock_to_nth_forecast_month(self):
        periods = ["2025-01", "2025-02", "2025-03", "2025-04", "2025-05", "2025-06"]
        df = _sequential_df({"MAT-1": [110.0, 120.0, 130.0, 140.0, 150.0, 160.0]}, periods=periods)
        df.loc[0, "Starting stock"] = 100.0

        result = MoMComparisonEngine.calculate_sequential(df, num_months=6)

        assert result["periods"] == ["Starting stock", "2025-06"]
        assert result["num_transitions"] == 1
        assert result["summary"][0]["start"] == pytest.approx(100.0)
        assert result["summary"][0]["end"] == pytest.approx(160.0)
        assert result["transitions"][0]["from_period"] == "Starting stock"
        assert result["transitions"][0]["to_period"] == "2025-06"
        assert result["transitions"][0]["rows"][0]["delta"] == pytest.approx(60.0)
    ...
    def test_delta_pct_computed_when_start_nonzero(self):
        ...
        assert result["summary"][0]["delta_pct"] == pytest.approx(50.0)

    def test_delta_pct_uses_absolute_negative_start(self):
        df = _sequential_df({"MAT-1": [-100.0, 50.0]}, periods=["2025-01", "2025-02"])
        result = MoMComparisonEngine.calculate_sequential(df, num_months=2)

        assert result["summary"][0]["delta_pct"] == pytest.approx(150.0)
        assert result["transitions"][0]["rows"][0]["delta_pct"] == pytest.approx(150.0)

    def test_num_transitions_is_single_start_to_target_comparison(self):
        df = _sequential_df({"MAT-1": [100.0, 200.0, 300.0]})
        result = MoMComparisonEngine.calculate_sequential(df, num_months=2)
        assert result["num_transitions"] == 1
    ...
    def test_num_months_capped_at_available_periods(self):
        ...
        assert result["num_transitions"] == 1
        assert result["periods"] == ["Starting stock", "2025-02"]
...
class TestCalculate:
    ...
    def test_delta_pct_uses_absolute_negative_previous(self):
