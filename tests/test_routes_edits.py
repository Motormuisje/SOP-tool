...
@pytest.mark.no_fixture
def test_reset_value_planning_edits_does_not_restore_valuation_params(edits_mock_app):
    edits_mock_app.make_session()
    edits_mock_app.engine.data.valuation_params = {"1": 9.0}
    edits_mock_app.sess["reset_baseline"] = {
        "results": True,
    ...
    payload = response.get_json()
    assert "restored_valuation_params" not in payload
    assert edits_mock_app.engine.data.valuation_params == {"1": 9.0}
