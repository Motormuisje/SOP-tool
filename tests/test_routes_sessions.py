...
def test_delete_last_active_session_leaves_empty_active_session(session_route_app):
    session_route_app.make_session("session-a", engine=None)
    session_route_app.set_active_session_id("session-a")

    response = session_route_app.client.delete("/api/sessions/session-a")

    assert response.status_code == 200, response.get_json(silent=True)
    assert response.get_json() == {"success": True, "active_session_id": None}
    assert session_route_app.sessions == {}
    assert session_route_app.get_active_session_id() is None
    assert session_route_app.save_calls
