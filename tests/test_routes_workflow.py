...
@pytest.mark.no_fixture
def test_multi_upload_warns_for_wrong_forecast_filename(workflow_multi_app, monkeypatch):
    monkeypatch.setattr(data_loader_module, "DataLoader", _make_good_loader())
    ...
    response = workflow_multi_app.client.post(
        ...
        data={
            ...
            "routing_file": (io.BytesIO(b"routing"), "routing.xlsx"),
            "stock_file": (io.BytesIO(b"stock"), "stock.xlsx"),
            "forecast_file": (io.BytesIO(b"forecast"), "delta april testen.xlsx"),
        },
        content_type="multipart/form-data",
    )
    ...
    assert payload["warnings"] == [
        'Wrong file for Forecast field: "delta april testen.xlsx" does not look like a Forecast file. '
        "Expected the filename to contain: forecast."
    ]
...
@pytest.mark.no_fixture
def test_calculate_returns_500_when_engine_run_raises(workflow_calculate_app, tmp_path, monkeypatch):
    class _RunRaisesEngine:
        def __init__(self, *args, **kwargs):
            self.data = SimpleNamespace(
                valuation_params=None,
                purchased_and_produced={},
    ...
    monkeypatch.setattr(workflow_module, "PlanningEngine", _RunRaisesEngine)
    workflow_calculate_app.add_session(file_path=str(tmp_path / "workbook.xlsm"))
    ...
    assert response.status_code == 500
    payload = response.get_json()
    assert "run failed" in payload["error"]
    assert "trace" in payload
