...
def _apply_edit_highlights(path: str, engine):
    ...
    # Build row lookups. Aux Column disambiguates rows that share material + line type.
    row_lookup = {}
    fallback_row_lookup = {}
    if mat_col_idx and lt_col_idx:
        for row_idx, row_data in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
            ...
            if mat_val and lt_val:
                simple_key = (str(mat_val), str(lt_val))
                fallback_row_lookup.setdefault(simple_key, row_idx)
                aux_val = row_data[aux_col_idx - 1] if aux_col_idx else ''
                row_lookup.setdefault((str(mat_val), str(lt_val), str(aux_val or '')), row_idx)
    ...
    for edit in all_edits:
        row_idx = row_lookup.get((edit['material_number'], edit['line_type'], edit['aux_column']))
        if row_idx is None:
            row_idx = fallback_row_lookup.get((edit['material_number'], edit['line_type']))
    ...
    ws_edits.append(['Line Type', 'Material Number', 'Material Name', 'Aux Column', 'Period',
                     'Original Value', 'New Value', 'Delta %'])
    for edit in all_edits:
        ws_edits.append([edit['line_type'], edit['material_number'], edit['material_name'],
                         edit['aux_column'], edit['period'], edit['original'], edit['new'], edit['delta_pct']])
...
def create_exports_blueprint(
    ...
    def get_mom_comparison():
        """Return starting-stock-to-target-month MoM comparison from the current run."""
        _, current_engine = get_active()
        if current_engine is None:
