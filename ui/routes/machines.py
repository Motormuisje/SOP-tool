...
def create_machines_blueprint(
    ...
    def get_machines():
        ...
        for group, machine_list in groups_map.items():
            ...
            # Group util is the available-capacity-weighted average of the per-machine util
            # values that come from the engine's L10 rows. Deriving it inline from L07/shift/avail
            # opens a second code path that drifts from L10 (e.g. when L10 hasn't picked up a
            # machine-tab shift_hours_override yet) — making a one-machine group disagree with
            # its sole child. For one machine, weighted average == that machine's util_pct.
            util_p = {}
            for period in periods:
                total_weighted = 0.0
                total_weight = 0.0
                for machine in machine_list:
                    avail_factor = machine['availability_by_period'][period] / 100.0
                    weight = avail_factor * machine['shift_hours']
                    machine_util_pct = machine['util_by_period'].get(period, 0.0)
                    total_weighted += machine_util_pct * weight
                    total_weight += weight
                util_p[period] = round((total_weighted / total_weight) if total_weight > 0 else 0.0, 1)
            eff_p = {}
            for period in periods:
